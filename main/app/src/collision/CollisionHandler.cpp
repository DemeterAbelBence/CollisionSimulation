...
void CollisionHandler::drawCollidersOf(const std::vector<SceneObject*>& sceneObjects, const Camera& camera) {
    for (SceneObject* sceneObject : sceneObjects) {
        ...
        if (cuboidCollider != nullptr) {
            ...
            torqueAxis.push_back(sceneObject->getBodyStateSolver()->Body.X);
            torqueAxis.push_back(cuboidCollider->getTransSides()[0].center);
            DebugDrawer::setMode(GL_LINES);
            DebugDrawer::setOverrideZ(1);
       ...
       /* auto nullCollider = dynamic_cast<NullCollider*>(sceneObject->getCollider());
        if (nullCollider != nullptr) {
            std::vector<glm::vec3> x = { sceneObject->getBodyStateSolver()->Body.X };
            DebugDrawer::setMode(GL_POINTS);
